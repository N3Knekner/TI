<!DOCTYPE html>
	...
	<body style="font-family: Segoe UI;">
		<button style="background-color: lightblue;" onclick="window.location.href = window.location.href.replace('veterinario.php','')">Voltar</button>
		...
		error_reporting(0);
			...
			try{
				...
				if (isset($_GET["Adicionar"])) {
					...
					echo "<script>
						if(confirm('Insirir um animal a este veterinario?')){
							window.location.href = window.location.pathname.replace('veterinario.php','')+'gado.php';
						}
						</script>";
				}
			...
			if (isset($_GET['alterar'])){
				echo "<script>
					if(confirm('deseja realmente alterar?'))window.location.href = 'alterarVeterinario.php?delete=".$_GET['alterar']."&table=".$_GET["table"]."&cod=".$col[0]."&tipo=".$_GET["tipo"]."'
					</script>";
			}
